"
The program must accept an integer matrix of size R*C and two integer X, Y as the input. The program must print the sum of integers in all X*Y non-overlapping submatrices as the output.
...
v=[list(map(int,input().split())) for i in range(s)]
rb,z=map(int,input().split())
for i in range(0,s-rb+1,rb):
    for j in range(0,f-z+1,z):
        ni=0
        for zx in range(i,i+rb):
            for d in range(j,j+z):
                ni+=v[zx][d]
        print(ni,end=" ")
